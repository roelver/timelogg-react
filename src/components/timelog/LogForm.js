...
const LogForm = function (props) {
   ...
   const taskDesc = formData
      ? formData.task
         ? formData.task
         : taskList.length > formData.dlogIdx
         ? taskList[formData.dlogIdx]
         : taskList.length > 0
         ? taskList[0]
         : undefined
      : undefined;
   ...
   const [startTimeHH, setStartTimeHH] = useState(
      formData ? getHH(formData.startTime) : 0
   );
   const [startTimeMM, setStartTimeMM] = useState(
      formData ? getMM(formData.startTime) : 0
   );
   const [endTimeHH, setEndTimeHH] = useState(
      formData ? getHH(formData.endTime) : 0
   );
   const [endTimeMM, setEndTimeMM] = useState(
      formData ? getMM(formData.endTime) : 0
   );
   ...
   const submitForm = (evt) => {
      ...
      if (formData && formData.dlogIdx && formData.tlogIdx) {
         ...
         if (task === updDlog.description) {
            ...
            const newDlog = allDaylogs.filter(
               (dlog) => dlog.description === task
            );
            if (!newDlog) {
   ...
   const formInvalid = () => {
      ...
      const timesOK =
         ...
         (parseInt(startTimeHH) === parseInt(endTimeHH) &&
            parseInt(startTimeMM) < parseInt(endTimeMM));
      const invalid = !(
         task &&
         timesOK &&
         (currentDate !== today() ||
            toTime(startTimeHH, startTimeMM, 0) < nowSecs())
      );
      return invalid;
   };
